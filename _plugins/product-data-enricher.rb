...
module Jekyll
  class ProductDataEnricher
    class << self
      ...
      MIN_TOTAL_CYCLES = 6
      MIN_HIDDEN_CYCLES = 3
      MAGIC_DAYS_VALUE = 4096 # For compute_days_toward_now_for_all_dates
      ...
      def flag_oldest_unmaintained_releases(page)
        ...
        handle_first_cycle_visibility(releases, hidden_cycles)
        handle_minimum_hidden_cycles(hidden_cycles)
      ...
      def mark_cycles_that_can_be_hidden(ordered_by_date_desc_releases)
        ...
        ordered_by_date_desc_releases.reverse.each do |cycle|
          if !cycle['is_maintained']
            if previous_cycle
      ...
      def handle_first_cycle_visibility(releases, hidden_cycles)
      ...
      def handle_minimum_hidden_cycles(hidden_cycles)
        if hidden_cycles.length > 0 && hidden_cycles.length < MIN_HIDDEN_CYCLES
          Jekyll.logger.debug TOPIC, "Less than #{MIN_HIDDEN_CYCLES} hidden cycles, unhide #{hidden_cycles.length} cycles"
      ...
      def compute_days_toward_now_for_all_dates(cycle)
        %w[eoas eol discontinued eoes].each { |field|
          ...
          if field_value.is_a?(Date)
            ...
            cycle[new_field_name] = field_value ? MAGIC_DAYS_VALUE * -1 : MAGIC_DAYS_VALUE
          end
        }
      end
